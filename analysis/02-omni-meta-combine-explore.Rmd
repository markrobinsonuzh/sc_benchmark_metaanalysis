...
find_concordance <- function(x) x %>% mutate(same = (first==second)+0) %>% pull(same) %>% mean()
...
rp <- retrieve_pairs(answers_g, "Number of criteria used for assessment",
               do_plot = FALSE, convert_int = TRUE, do_categorical = FALSE, replace = -1)
rp %>% mutate(same = (first==second)+0) %>% pull(same) %>% mean()


rp <- retrieve_pairs(answers_g, "Degree to which code to re-run benchmark is extensible",
               do_plot = FALSE, convert_int = TRUE, do_categorical = FALSE, replace = -1)
rp %>% mutate(same = (first==second)+0) %>% pull(same) %>% mean()
...
rp <- retrieve_pairs(answers_g, "For the methods benchmarked, what is/are the primary language/s they are written in?",
               do_plot = FALSE, convert_int = TRUE, do_categorical = FALSE, replace = -1)
rp %>% mutate(same = (first==second)+0) %>% pull(same) %>% mean()
